function accountType(evt) {
  evt.classList.add("bg-active");
  console.log(evt.name, this);
}
...
// slider.noUiSlider.set([10000, 20000]);
...
//tagify for country test

var tagifyForCountry = new Tagify(
  document.querySelector("input[name=tags3-1]"),
  {
    delimiters: null,
    mode: "select",
    templates: {
...
tagifyForCountry.on("click", function (e) {
  console.log(e.detail);
});

tagifyForCountry.on("remove", function (e) {
  console.log(e.detail);
});

tagifyForCountry.on("add", function (e) {
  console.log("original Input:", tagifyForCountry.DOM.originalInput);
  console.log(
    "original Input's value:",
    tagifyForCountry.DOM.originalInput.value
  );
  console.log("event detail:", e.detail);
});

// add the first 2 tags and makes them readonly
var tagsToAdd = tagifyForCountry.whitelist.slice(0, 0);
tagifyForCountry.addTags(tagsToAdd);

//tagify for Language test

var adsLanguage = document.querySelector("input[name=adsLanguage]");
new Tagify(adsLanguage, {
  whitelist: ["Hindi", "English", "Punjabi"],
  dropdown: {
    position: "input",
    enabled: 0,
  },
